...
int main() {
    ...
    OptionsWakeSleep options_wake_sleep;
    ...
    // ***************
    // MARK: Binary vs prob hiddens and visibles
    // ***************

    options_wake_sleep.is_asleep_visible_binary = true;
    options_wake_sleep.is_asleep_hidden_binary = true;
    options_wake_sleep.is_asleep_visible_binary_final = true;
    options_wake_sleep.is_asleep_hidden_binary_final = false;

    // ***************
    // MARK: - Gibbs vs mean field
    // ***************
    
    options_wake_sleep.gibbs_sample_awake_phase = false;
    options_wake_sleep.gibbs_sample_awake_phase_hidden_binary = true;
    ...
    // No markov chains
    int no_markov_chains_awake = 5; // 5
    int no_markov_chains_asleep = 5;
    
    // Opt solver class
    OptProblemStatic opt(dbm.latt);

    // Set no markov chains
    dbm.latt->set_no_markov_chains(MCType::AWAKE, no_markov_chains_awake);
    dbm.latt->set_no_markov_chains(MCType::ASLEEP, no_markov_chains_asleep);
    
    // Params
    int no_opt_steps = 10000; // 10000
    int no_gibbs_sampling_steps = 5;
    int no_mean_field_steps = 5;
    
    // ***************
    // MARK: - Iterate over timepoints
    // ***************
    
    for (auto timepoint=0; timepoint<=300; timepoint+=10) {
        ...
        // Read means
        double val = 0.0;
        auto init_means = read_means("../../../rossler_stoch_sims/means_v1/"+pad_str(timepoint+1,4)+".txt");
        ...
        // Other ixn params
        // if (timepoint == 0) { //  || timepoint == 25) {
        for (auto ixn: dbm.ixns) {
            if (ixn->get_name() == "bX1" || ixn->get_name() == "bY1" || ixn->get_name() == "bZ1") {
                ixn->set_val(0.0);
            } else if (ixn->get_name() == "wAX1") {
                // ixn->set_val(0.01);
                ixn->set_val(1.0);
            } else if (ixn->get_name() == "wBY1") {
                // ixn->set_val(0.01);
                ixn->set_val(1.0);
            } else if (ixn->get_name() == "wCZ1") {
                // ixn->set_val(0.01);
                ixn->set_val(1.0);
        ...
        for (auto i_batch=1; i_batch<100; i_batch++) {
            ...
            fnames.add_fname(FName("../../../rossler_stoch_sims/data_v1/lattice_v" + pad_str(i_batch,3) + "/lattice/"+pad_str(timepoint,4)+".txt",binary_read));
        };
        ...
        for (auto opt_step=1; opt_step<=no_opt_steps; opt_step++) {
            ...
            // Solve
            if (opt_step == 1) {
                opt.solve_one_step(opt_step,no_mean_field_steps,100,fnames,options,options_wake_sleep);
            } else {
                opt.solve_one_step(opt_step,no_mean_field_steps,no_gibbs_sampling_steps,fnames,options,options_wake_sleep);
            };
            
            // Params
            /*
            for (auto ixn: dbm.ixns) {
                cout << ixn->get_name() << " " << ixn->get_val() << endl;
            };
             */
